...
namespace BracketSpace\Notification\BuddyPress\Components\Carrier;
 ...
 * BuddyPress Notification Carrier
 */
class BuddyPressNotification extends AbstractCarrier {
	...
	public function send( Triggerable $trigger ) {
		...
		foreach ( $data['parsed_recipients'] as $receiver_id ) {
			$notification_id = bp_notifications_add_notification( [
				'user_id'          => $receiver_id,
				'component_name'   => 'notification-buddypress',
				'component_action' => $trigger->get_slug(),
				'is_new'           => 'unread' === $data['state'],
				'allow_duplicate'  => true,
			] );

			if ( $notification_id ) {
				bp_notifications_add_meta( $notification_id, 'notification_content', $data['content'] );
				if ( ! empty( $data['link'] ) ) {
					bp_notifications_add_meta( $notification_id, 'notification_link', $data['link'] );
				}
			}
		}
